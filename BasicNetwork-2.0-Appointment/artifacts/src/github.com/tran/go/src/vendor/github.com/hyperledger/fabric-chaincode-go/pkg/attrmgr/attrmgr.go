...
func (mgr *Mgr) GetAttributesFromIdemix(creator []byte) (*Attributes, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal appointment invoker's identity: %s", err)
	}
	idemixID := &msp.SerializedIdemixIdentity{}
	err = proto.Unmarshal(sid.IdBytes, idemixID)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal appointment invoker's idemix identity: %s", err)
	}
	// Unmarshal into attributes object
	attrs := &Attributes{
	...
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal appointment invoker's ou: %s", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal appointment invoker's role: %s", err)
	}
	var roleStr string
	switch role.Role {
	case 0:
